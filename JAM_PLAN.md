...
Checkpoint G — resilience and motion control: the footer and P key now cycle persistent System → Reduced → Full motion modes; only presentation changes, while deterministic physics, score and launch state stay shared. The release audit enforces both that control and the localhost-only diagnostic gate. On desktop, all three states exposed correct labels and assistive states, Reduced persisted through reload, and score/launch state remained 0/8. An active keyboard aim survived the simulated background boundary correctly by cancelling without spending a launch, and forced WebGL loss moved from unavailable/lost back to available/restored at 161 draw calls. At 390×844 all three 44-pixel footer controls fit without overflow; a real one-pointer opening landed on Ember for 1,000, Reset restored Meadow and 8 launches, and the console had no warnings or errors. Build `20260814-ob12` passes all 108 tests, syntax checks and the release audit. No public build, production service, platform entry, capture upload or announcement was created.

Checkpoint H — adaptive performance recovery: the render-resolution policy is now a pure tested state transition rather than an unverified frame-loop branch. A slow visible two-second sample drops the cap by 0.25 immediately; four clearly smooth samples restore one step, while a middle band and hidden pages reset recovery pressure to prevent oscillation. Browser diagnostics proved desktop cap 2.00 → 1.75 → 2.00 and portrait cap 1.50 → 1.25 → 1.50 without changing the 0 score or 8-launch state. A subsequent real 390×844 one-pointer shot landed on Ember for 1,000, stayed within budget at 166 maximum draw calls, had no overflow and produced no console warnings or errors. Build `20260814-ob13` passes all 112 tests, syntax checks and the strengthened release audit. No public build, production service, platform entry, capture upload or announcement was created.

Checkpoint I — public GitHub Pages playtest: after explicit user approval, verified PR #20 was merged and GitHub Pages was enabled from `main` at `https://southers.github.io/ORBITBREAK/`, matching the established WORLDSEED hosting model. GitHub reported commit `af7958a` built successfully in 41.2 seconds; the public HTML, main module, performance module and vendored Three.js runtime all returned HTTP 200. Desktop loaded Breaker's Reach at 8 launches, 0 score and 161 draw calls. At 390×844 all footer controls fit, a real one-pointer opening landed on Ember for 1,000 with 7 launches remaining, maximum draw calls stayed within budget at 169 and the console was clean. The public leaderboard remains honestly offline. No jam submission, production leaderboard resource, media upload or announcement was created.
