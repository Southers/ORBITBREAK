...
- Build `20260818-ob115` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob115`): the author screenshot showed the parked Orbitbreaker still as a mint vertical bar because the landed camera's up is world Z and the hull's nose had been aligned with that axis. The courier now lies across the crust (camera-up × surface normal), with a dumpier winged hull, so the close-up reads a tiny ship beside the Runner rather than a pole.
