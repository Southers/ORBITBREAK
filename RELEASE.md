...
- Build `20260819-ob136` is the local candidate after live ob135 Command skim-lock, hull drag-back launching, and a Command ring that did not match the landing. Pages cache-bust is `20260819-ob136`.
- Highlight kill (`ob136`): Haven → Ember → Bastion (cage smash) → Glasswing → Command parked the courier ~80px off the rim with the camera locked and no grab, W, or cage tap; only R recovered. Command now uses the skim catch so that throw lands or recaptures. Dragging back onto the on-screen hull after a committed aim cancels instead of firing a full-power Haven→Ember throw. The Command endpoint ring samples the orbit at predicted collision time so it cannot sit on Command while the ship hits Glasswing. Grove Escape cancel, Zoom ±, Command EXPOSE, rim cages, drag-walk and shared cache-bust stay.
- Build `20260819-ob136` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Build `20260819-ob135` is the local candidate after live ob134 Grove camera/launch jam, vanishing pause Zoom, missing Command chip and sparse cages. Pages cache-bust is `20260819-ob135`.
- Highlight kill (`ob135`): pulling the parked hull on Grove zoomed to the neighbourhood map and never launched because the 52px screen cancel disc covered the shrunken planet. Committed aim now cancels only on a short world-space drag (or Escape); cancel, lost capture, Q/E walk and Resume snap the landed close-up so the hull stays grabbable. Pause Zoom −/+ keep their own Scout scale and stay visible. Command opens after two inner worlds plus further travel and keeps a chip after leaving the start cluster. Occupied worlds, including Grove, keep a tappable equatorial cage in a slightly wider landed frame. Drag-walk, shared cache-bust, Q/E 14°, HERE chip and F5 boot stay.
- Build `20260819-ob135` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob134`): live ob132 crust-drag launched the ship 3/3 because the 3.2× in-flight pick sphere covered the landed disc; empty crust now walks, hull-pull still flies, and cage tap still smashes. Q/E/T/F step 14° with a faster hold so a far-side cage is reachable. Spent mid-flight Space toasts BREAK ALREADY USED. Scout names HERE on the current world and calls Seedstone a launch. Pause motion reads Motion: system/reduced/full. Cache-bust, Command expose, outer leave, skim recapture and toy props stay. Pages cache-bust is `20260819-ob134`.
- Build `20260819-ob133` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob133`): live ob132 booted black for returning players because `main.js` still imported `sector.js?v=ob131`; a 6-world Grove–Ember–Frost–Bastion–outer run left Command LOCKED · MOVING; max-power throws from the outer pine/white well CLEAN LANDED home. One shared cache-bust token now stamps every module import. Command opens after a live neighbourhood plus further travel, including outposts, and stays open if the Warden silences an inner world. Origin ignore lasts 2.4 plus 0.8 s on live flight, prediction and replay. Pages cache-bust is `20260819-ob133`.
- Build `20260819-ob132` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob132`): a second ob130 run restored Grove and Ember but every Grove throw CLEAN LANDED home and W after the first hop only stretched a stub to already-linked neighbours. Grove/Ember wells sat above the 12.5 cap from the landed face. Launch speed is 16.5; origin ignore lasts 1.2 in live flight and prediction; A/D hold-steers; Enter still launches. Command still stays LOCKED until a live neighbourhood plus further travel. Pages cache-bust is `20260819-ob132`.
- Build `20260819-ob131` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob131`): live ob130 died on Bastion→Frost (camera parked, courier skimming, keys dead), Command stayed LOCKED after a 5-world tour, and linked worlds still showed planet-filling cream slabs, Tide's white pole, and giant orange Bastion cages. Near-surface skims now recapture; Command opens after a live neighbourhood plus further travel; overlay props no longer stack on merged landmarks; cages are toy-scale; Enter launches an armed aim. Pages cache-bust is `20260819-ob131`.
- Build `20260819-ob130` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob130`): a second complete-run playtest on live ob128 still could not leave the linked garden or find Command. Keyboard aim no longer snaps to already-linked neighbours; A/D is 12° free-steer from the launch face; W/S start at mid power and change a visible aim stem plus arc width; the trajectory stays drawn while aiming; Scout/aim always show one Command label (LOCKED or EXPOSED); cage copy only appears while a tappable equatorial cage is up; How to play still names Q/E and A/D/W/S. Pages cache-bust is `20260819-ob130`.
- Build `20260819-ob129` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob129`): live complete-run playtest could not finish — Grove/Frost spawned world-piercing capsules and crust-sized slabs, cages never appeared unless the gold beacon landed, ship-pull from the parked hull walked the globe, and Command was unreachable because A/D stepped 2° while W/S reframed the aim camera. Toy props now plant and scale to the globe; occupied worlds spawn a bright equatorial cage on dock; ship-pull hits the hull with a visible halo; keyboard steer is 8°/hold-to-steer and aim zoom no longer tracks the arc; landed camera stays closer and snaps back to the courier. How to play names Q/E and A/D/W/S. Pages cache-bust is `20260819-ob129`.
- Build `20260819-ob128` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (How to play overlay, pause sheet, ship fuel lights, Warden vessel telegraph).
- Highlight kill (`ob128`): sampled ElevenLabs voice/SFX/music play from committed `assets/audio/` files after the first gesture, with mute, reduced-motion music-off, and the in-engine bed as fallback. The browser never calls ElevenLabs. Last-pass +Z camera, planet spin, readable life, cage destroy, How to play, copy boards, and the no glow-disc/grid/mint-ribbon kills stay.
- Highlight kill (`ob127`): live 390×844 playtest still felt like tap-does-nothing / swipe-launches because the on-globe ship halo was `min(72, worldRadius*0.38)` — a ~210px disc, 12% of the viewport, so a swipe that started just off a cage armed launch. Phone close-up now grabs the parked hull only (small disk around the mesh, not a fraction of a filling planet); the 96px empty-space halo stays for small scout worlds. Cage still wins on/near a cage; walk still owns the rest of the crust. Copy no longer says "walk near then tap" — there is no proximity gate. Haven limb houses are unchanged. Planet spin, +Z camera, buildings-on-crust, How to play, and ship-not-eating-the-planet stay.
