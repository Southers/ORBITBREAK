...
- Build `20260816-ob98` passes deterministic tests, syntax checks, the three-sector release audit and the 200-run validator benchmark.
- Breaker's Reach is a twelve-body sector: Haven, Ember and Grove stay the inner neighbourhood, Spindle, Quarry, Mirage, Frost, Tide and Bastion wait behind the veil, and the Ledge, Cinder and Glasswing relay shards stage the mandatory slingshot legs. Aiming frames the current world's neighbours so the bigger map stays readable; Scout zoom-out reaches 3.85. Golden complete replay derives score 63,400 over eight launches on content `breaker-reach-8`.
