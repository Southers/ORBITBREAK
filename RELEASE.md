...
- Build `20260818-ob106` passes deterministic tests, syntax checks, lint, typecheck, the three-sector release audit, the Playwright boot smoke, the 200-run validator benchmark (~52ms median per verified replay), and the zero-HUD play contract (pause sheet, ship fuel lights, Warden vessel telegraph).
- Toy-diorama scale: people, trees, houses, workshops, docks, pack beasts and occupation pylons sit as miniature props. House/workshop profiles dropped from ~0.9–1.4 to ~0.32–0.44 so cottages no longer fight world radius. World radii, gravity, scoring and flight physics are unchanged.
