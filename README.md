...
The public build is one dense Breaker's Reach run: scout, walk the rim, grab the ship to aim, slingshot, **Break** in flight, land, watch worlds wake, then outrun the Warden. Aiming frames the neighbourhood on a brighter map; pinch, wheel or the zoom buttons go from a close world to the whole authored Reach. Haven, Ember and Grove are the inner neighbourhood. Completing that cluster recedes the stillness veil; landing a further world (Tide, Frost, Bastion, Spindle, Quarry or Mirage) reveals the Warden. Gold circuits protect, push pursuit back and crack Command's shields. After the first live link, Ember keeps one leftover tooth to teach **Cut**; Bastion and Command still use the full cage. After the first loop, a gold ghost marks the next closing edge. Command lock is the finale gift once both shields crack. Bonus fuel is not a fail timer. Only a defeated Warden banks a ranked run. Local personal-best ghosts are optional; the public Breakers Board stays offline until a production endpoint is deliberately approved.
