...
- Exposed Command finish (checkpoint 31 / live ob135): Command uses the same skim catch as recapture so a throw that parks ~80px off the rim lands or returns control instead of freezing in flight. After the planning camera jumps, dragging back onto the on-screen hull still cancels (28px), while a long pull on a shrunken Grove still launches. The Command endpoint ring sits on the body at predicted collision time, not the current orbit pose, so prediction and live flight agree.
