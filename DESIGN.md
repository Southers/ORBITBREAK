...
- Alive crust pass after playtest `20260818-ob121`: inhabited worlds keep denser biome scatter, culture-true overlay props and local skiffs even while isolated or tyrant, so Haven/Ember/Grove/Frost/Tide/Bastion read as different toy worlds from the landed +Z camera. Walking near an authored crust discovery banks once into `discoveryScore` with a quiet `Haven 2/4 · garden relay` toast and no map pins or collectathon HUD. Ranked flight totals stay on the shared simulation; exploring cannot desync a verified replay. Shards stay sparse. Planet-spin-under-walker and runner-planted-on-crust are unchanged.
