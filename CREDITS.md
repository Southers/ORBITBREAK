...
Sampled voice, SFX and music under `assets/audio/` are generated with [ElevenLabs](https://elevenlabs.io/). The currently committed production clips use Daniel for the Warden broadcast and Charlie for the Runner and world replies. The project owner selected Voice Design candidate 3 as the replacement `ORBITBREAK Warden` profile; its clean source and ORBITBREAK-authored radio/robot master remain ungenerated until a separate production run is approved. Audio is produced in GitHub Actions from `secrets.ELEVENLABS_API_KEY` and committed as static files. The playable Pages build never calls ElevenLabs. Until replacement files are present, the legacy sampled Warden clips and procedural Web Audio remain available. All other geometry, materials, particles, interface beds and in-engine tones are generated in code.
