...
test('playable sources never call ElevenLabs and the workflow never echoes the secret', () => {
  ...
  assert.equal(AudioSource.includes('this.resumeContext()'), true);
  assert.equal(AudioSource.includes('playGestureUnlock()'), true);
  assert.equal(AudioSource.includes('playHtmlMediaUnlock()'), true);
  assert.equal(AudioSource.includes('new AudioConstructor('), true);
  assert.equal(AudioSource.includes("getClipById('sfx/ui-continue')"), true);
  assert.equal(AudioSource.includes('getAudioAssetUrl'), true);
  assert.equal(AudioSource.includes("preload = 'auto'"), true);
  assert.equal(AudioSource.includes('playsInline = true'), true);
  assert.equal(AudioSource.includes('whenContextRunning()'), true);
  assert.equal(Workflow.includes('secrets.ELEVENLABS_API_KEY'), true);
  assert.equal(Workflow.match(/secrets\.ELEVENLABS_API_KEY/g)?.length, 1);
  assert.equal(Workflow.includes('workflow_dispatch'), true);
  assert.equal(Workflow.includes('type: boolean'), true);
  ...
  assert.equal(Workflow.includes('actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02'), true);
  assert.equal(Workflow.includes('actions/download-artifact@d3f86a106a0bac45b974a628896c90dbdf5c8093'), true);
  assert.equal(Workflow.includes('audio/elevenlabs-assets-${{ github.run_id }}-${{ github.run_attempt }}'), true);
  assert.equal(Workflow.includes('git add -- assets/audio'), true);
  assert.equal(Workflow.includes('git push -u origin audio/elevenlabs-assets --force'), false);
  assert.equal(/uses:\s+[^\n]+@v\d/.test(Workflow), false);
  assert.equal(/uses:\s+[^\n]+@v\d/.test(CiWorkflow), false);
  assert.equal(PublishJob.includes('ELEVENLABS_API_KEY'), false);
