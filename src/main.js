...
import { WorldseedAudio } from './audio.js?v=20260819-ob140';
import {
  SurfaceGestureModes,
...
} from './preferences.js?v=20260819-ob140';
import {
...
} from './performance.js?v=20260819-ob140';
import { addEnvironment } from './environment.js?v=20260819-ob140';
import { createWorldVisuals } from './world-geometry.js?v=20260819-ob140';
import { createLivingWorldVisuals } from './living-world-visuals.js?v=20260819-ob140';
import { createWardenVisuals } from './warden-visuals.js?v=20260819-ob140';
import { createPlayerVisuals } from './player-visuals.js?v=20260819-ob140';
import { createStoryDirector } from './story-director.js?v=20260819-ob140';
import { createHud } from './hud.js?v=20260819-ob140';
import { createAimPreview } from './aim-preview.js?v=20260819-ob140';
import { createLandingDirector } from './landing-director.js?v=20260819-ob140';
import { createCameraController } from './camera-controller.js?v=20260819-ob140';
import { createInputController } from './input-controller.js?v=20260819-ob140';
import { createHostileSurface } from './hostile-surface.js?v=20260819-ob140';
import { createScanner } from './scanner.js?v=20260819-ob140';
import { createRoutePresentation } from './route-presentation.js?v=20260819-ob140';
import { createRecordsUi } from './records-ui.js?v=20260819-ob140';
import { createFrameVisuals } from './frame-visuals.js?v=20260819-ob140';
import { createRestorationVisuals } from './restoration-visuals.js?v=20260819-ob140';
import { EffectComposer } from '../vendor/postprocessing/EffectComposer.js?v=20260819-ob140';
import { RenderPass } from '../vendor/postprocessing/RenderPass.js?v=20260819-ob140';
import { UnrealBloomPass } from '../vendor/postprocessing/UnrealBloomPass.js?v=20260819-ob140';
import { OutputPass } from '../vendor/postprocessing/OutputPass.js?v=20260819-ob140';
...
} from './campaign.js?v=20260819-ob140';
...
import {
  ...
  predictTrajectory,
  simulatePhysicsStep,
} from './physics.js?v=20260819-ob140';
import {
...
} from './sim-constants.js?v=20260819-ob140';
import {
...
} from './flight-resolver.js?v=20260819-ob140';
import { createLeaderboardClient, resolveLeaderboardBaseUrl } from './leaderboard-client.js?v=20260819-ob140';
import {
  connectRelayWorlds,
  ...
  listRelayCircuits,
  listRelayLinks,
} from './network.js?v=20260819-ob140';
import {
  WardenPursuitEvents,
  createWardenPursuitState,
} from './warden.js?v=20260819-ob140';
import {
  loadPersonalBest,
} from './records.js?v=20260819-ob140';
import {
...
import {
  ...
  recordReplayBurn,
  recordReplayLaunch,
} from './replay.js?v=20260819-ob140';
import {
  consumeDueReplayBurn,
  consumeDueReplayLaunch,
} from './replay-playback.js?v=20260819-ob140';
import {
  createRunState,
  failRunToWarden,
  releaseRunLaunch,
  settleRunFlight,
} from './run.js?v=20260819-ob140';
import {
  bankFlightScore,
  createScoreState,
...
} from './scoring.js?v=20260819-ob140';
...
GameCanvas.dataset.build = '20260819-ob140';
...
function openCommandWorldRoute() {
...
let CancelPlayAimForPause = null;
let RefreshPauseZoomControls = null;

function setPauseSheetOpen(IsOpen) {
  if (IsOpen && isBlockingMenuOpen()) {
    return;
  }
  if (IsOpen && typeof CancelPlayAimForPause === 'function') {
    CancelPlayAimForPause();
  }
  ...
  if (IsPauseSheetOpen) {
    if (typeof RefreshPauseZoomControls === 'function') {
      RefreshPauseZoomControls();
    }
...
function isLiveFurtherReach(LiveWorldIdentifiers = listLiveWorldIdentifiers()) {
  return hasTravelledFurther(
    LiveWorldIdentifiers,
    ActiveSystem.innerClusterWorldIdentifiers,
    ActiveSystem.furtherReachWorldIdentifiers,
    ActiveSystem.commandWorldIdentifier,
  );
}
...
function resolveWardenAfterResolvedFlight({ firstCircuitClosed = false, circuit = null } = {}) {
  ...
  const CommandWorldJustExposed = (
    Resolution.isWorldheartOpen === true && openCommandWorldRoute()
...
RefreshPauseZoomControls = refreshPlanningZoomControls;
...
function simulateSeedFixedStep() {
  ...
  const StepResult = advanceSimulatedFlightStep({
    ...
    flightElapsedSteps: Math.max(1, Math.round(FlightElapsedSeconds / FixedPhysicsStepSeconds)),
  ...
  if (CollisionBody?.definition.kind === 'worldheart') {
    ...
    recoverSeedFromVoid('CAUGHT IN ORBIT');
    return;
  }
  ...
  if (StepResult.outOfBounds || StepResult.orbitTrapped) {
    if (StepResult.orbitTrapped) {
      const TrappedOnCommand = FlightOrbitTrapState.worldIdentifier === WorldheartDefinition.id
        || FlightOrbitTrapState.skimWorldIdentifier === WorldheartDefinition.id;
      if (TrappedOnCommand) {
        const CommandPosition = calculateBodyPositionAtTime(
          WorldheartDefinition,
          PhysicsElapsedTimeSeconds,
        );
        const CommandCatch = getTacticalBodyCollisionRadius(WorldheartDefinition)
          + RunnerRadius
          + FlightSkimClearance;
        const CommandDistance = Math.hypot(
          SeedPhysicsState.position.x - CommandPosition.x,
          SeedPhysicsState.position.y - CommandPosition.y,
        );
        if (
          CommandDistance <= CommandCatch
          && attachSeedToWorldheart(SeedPhysicsState.position, CommandPosition)
        ) {
          return;
        }
      }
